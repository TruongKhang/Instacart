...
def get_model(num_users, num_items, layers = [20,10], reg_layers=[0,0]):
    ...
    latent_aux_user_input = Dense(5, name='latent_aux_user_input')(aux_user_input)
    mlp_full_user_input = merge([MLP_Embedding_User(user_input), latent_aux_user_input], mode='concat')

    aux_item_input = Input(shape=(2,), name='aux_item_input')
    latent_aux_item_input = Dense(5, name='latent_aux_item_input')(aux_item_input)
    mlp_full_item_input = merge([MLP_Embedding_Item(item_input), latent_aux_item_input], mode='concat')
    
    # Crucial to flatten an embedding vector!
    user_latent = Flatten()(mlp_full_user_input)
    item_latent = Flatten()(mlp_full_item_input)

...
def get_model(num_users, num_items, latent_dim, regs=[0,0]):
    ...
    mf_full_user_input = merge([MF_Embedding_User(user_input), latent_aux_user_input], mode='concat')

    aux_item_input = Input(shape=(2,), name='aux_item_input')
    latent_aux_item_input = Dense(5, name='latent_aux_item_input')(aux_item_input)
    mf_full_item_input = merge([MF_Embedding_Item(item_input), latent_aux_item_input], mode='concat')

    # Crucial to flatten an embedding vector!
    user_latent = Flatten()(mf_full_user_input)
    item_latent = Flatten()(mf_full_item_input)
